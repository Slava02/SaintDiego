...
import (
	...
	"github.com/Slava02/SaintDiego/backend/volunteers/internal/server"
	"github.com/Slava02/SaintDiego/backend/volunteers/internal/usecases/volunteers"
	_ "github.com/go-sql-driver/mysql"
	"go.uber.org/zap"
	"golang.org/x/sync/errgroup"

	v1 "github.com/Slava02/SaintDiego/backend/volunteers/internal/server/v1"
)

const (
	nameMain    = "main"
	serviceName = "volunteers"
)
...
func run() error {
	...
	volunteersRepo, err := volunteers_repo.NewVolunteerRepository(volunteers_repo.NewOptions(db))
	if err != nil {
		return fmt.Errorf("init volunteers repo: %v", err)
	}

	// Usecases

	usecase, err := volunteers.New(volunteers.NewOptions(volunteersRepo))
	if err != nil {
		lg.Error("init volunteers usecase", zap.Error(err))
		return fmt.Errorf("init volunteers usecase: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("init volunteers service: %v", err)
	}
