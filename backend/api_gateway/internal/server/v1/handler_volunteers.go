...
type IVolunteerUC interface {
	...
	PutVolunteersTgId(ctx context.Context, tgId int64, req *volunteers.UpdateVolunteerRequest) (*models.Volunteer, error)
}
...
func (h Handlers) PutVolunteersTgId(ctx echo.Context, tgId int64) error {
	...
	volunteer, err := h.volunteerUC.PutVolunteersTgId(ctx.Request().Context(), tgId, &req)
	if err != nil {
		return ctx.JSON(http.StatusInternalServerError, err.Error())
	}
