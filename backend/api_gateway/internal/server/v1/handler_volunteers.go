...
type IVolunteerUC interface {
	PostVolunteers(ctx echo.Context, req volunteers.CreateVolunteerRequest) (*models.Volunteer, error)
	GetVolunteersTgId(ctx echo.Context, tgId int64) (*models.Volunteer, error)
	PutVolunteersTgId(ctx echo.Context, tgId int64, req volunteers.UpdateVolunteerRequest) (*models.Volunteer, error)
}
...
func (h Handlers) PostVolunteers(ctx echo.Context) error {
	...
	volunteer, err := h.volunteerUC.PostVolunteers(ctx, req)
	if err != nil {
...
func (h Handlers) GetVolunteersTgId(ctx echo.Context, tgId int64) error {
	volunteer, err := h.volunteerUC.GetVolunteersTgId(ctx, tgId)
	if err != nil {
		return ctx.JSON(http.StatusInternalServerError, err.Error())
	}
...
func (h Handlers) PutVolunteersTgId(ctx echo.Context, tgId int64) error {
	...
	volunteer, err := h.volunteerUC.PutVolunteersTgId(ctx, tgId, req)
	if err != nil {
		return ctx.JSON(http.StatusInternalServerError, err.Error())
	}
