...
CREATE TABLE IF NOT EXISTS `client_settings` (
    ...
    CONSTRAINT `FK_client_settings_client` FOREIGN KEY (`client_id`) REFERENCES `client` (`id`) ON DELETE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;
--bun:split
...
--bun:split
CREATE TABLE IF NOT EXISTS `service_type_settings` (
    `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    PRIMARY KEY (`id`),
    KEY `IDX_service_type_settings_type_id` (`service_type_id`),
    CONSTRAINT `FK_service_type_settings_type` FOREIGN KEY (`service_type_id`) REFERENCES `service_type` (`id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;
--bun:split
...
CREATE TABLE IF NOT EXISTS `event` (
    ...
    `remaining_capacity` int(11) NOT NULL DEFAULT capacity,
...
CREATE TABLE IF NOT EXISTS `client_service_history` (
    ...
    KEY `IDX_client_service_history_type_id` (`service_type_id`),
    CONSTRAINT `FK_client_service_history_client` FOREIGN KEY (`client_id`) REFERENCES `client` (`id`) ON DELETE CASCADE,
    CONSTRAINT `FK_client_service_history_type` FOREIGN KEY (`service_type_id`) REFERENCES `service_type` (`id`) ON DELETE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;
--bun:split
CREATE TRIGGER update_remaining_capacity_insert
AFTER
INSERT ON event_client FOR EACH ROW BEGIN
UPDATE event e
...
AFTER
UPDATE ON event_client FOR EACH ROW BEGIN IF NEW.status != OLD.status THEN
UPDATE event e
SET e.remaining_capacity = e.capacity - (
        SELECT COUNT(*)
        FROM event_client ec
...
AFTER DELETE ON event_client FOR EACH ROW BEGIN
UPDATE event e
SET e.remaining_capacity = e.capacity - (
        SELECT COUNT(*)
        FROM event_client ec
...
END;
--bun:split
CREATE TRIGGER set_service_type BEFORE
INSERT ON event FOR EACH ROW
...
--bun:split
CREATE TRIGGER after_event_client_update
AFTER
INSERT ON event_client FOR EACH ROW BEGIN
INSERT INTO client_service_history (client_id, service_type_id, last_attended_date)
VALUES (
        NEW.client_id,
        (
            SELECT service_type_id
...
UPDATE last_attended_date = IF(NEW.status = 'attend', NOW(), last_attended_date);
END;